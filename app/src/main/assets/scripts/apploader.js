...
var ConchNode = (function() {
  ...
  ConchNode.prototype.custom = function(t) {
    ...
    } else _Buffer.oneInt(this, 20, t.id);
  };
...
var ConchGraphics = (function() {
  ...
  ConchGraphics.prototype.fillText = function(
    text,
    x,
    y,
    font,
  ...
  ConchGraphics.prototype.fillBorderText = function(
    text,
    x,
    y,
    font,
    ...
    if (borderColor) {
      ...
      if (decorationLine > 0) {
        font +=
          ' ' + 0 + ' ' + '#000000' + ' ' + decorationLine + ' ' + fillColor;
      }
    }
  ...
  ConchGraphics.prototype._fands = function(fillColor, lineColor, lineWidth) {
    ...
    if (fillColor) {
      ...
      if (isNaN(fd)) {
        console.error(
          'graphics drawRect fillcolor:' + fillColor + ' not support'
        );
        fd = 0xffffffff;
      }
    } else {
    ...
    if (lineColor) {
      ...
      if (isNaN(ld)) {
        console.error(
          'graphics drawRect lineColor' + lineColor + ' not support'
        );
        ld = 0xffffffff;
      }
    } else {
  ...
  ConchGraphics.prototype.drawRect = function(
    x,
    y,
    width,
    height,
  ...
  ConchGraphics.prototype.drawCircle = function(
    x,
    y,
    radius,
    fillColor,
  ...
  ConchGraphics.prototype.drawPie = function(
    x,
    y,
    radius,
    startAngle,
  ...
  ConchGraphics.prototype.drawImageM = function(
    img,
    sx,
    sy,
    sw,
  ...
  ConchGraphics.prototype.drawPoly = function(
    x,
    y,
    points,
    fillColor,
  ...
  ConchGraphics.prototype.drawLine = function(
    x1,
    y1,
    x2,
    y2,
  ...
  ConchGraphics.prototype._drawPs = function(
    fid,
    x,
    y,
    points,
  ...
  ConchGraphics.prototype.drawLines = function(
    x,
    y,
    points,
    lineColor,
  ...
  ConchGraphics.prototype.drawCurves = function(
    x,
    y,
    points,
    lineColor,
  ...
  ConchGraphics.prototype.setSkinMesh = function(
    ib,
    vb,
    eleNum,
    iStart,
  ...
  ConchGraphics.prototype.drawImageS = function(
    img,
    sx,
    sy,
    sw,
...
var PlatformBase = (function() {
  ...
  PlatformBase.prototype.call = function(
    methodName,
    p1,
    p2,
    p3,
  ...
  PlatformBase.prototype.callWithBack = function(
    callback,
    methodName,
    p1,
    p2,
...
class PlatformClass extends PlatformBase {
  ...
  newObject(p1, p2, p3, p4, p5, p6, p7, p8, p9) {
    ...
    a.splice(0, 0, '<init>');
    const rs = new PlatformObj(this);
    rs.init.apply(rs, a);
    return rs;
  }
}
PlatformClass.createClass = clsName =>
...
class ConchFps extends ConchNode {
  ...
  render(f) {
    let t = conch.getCountVertext();
    let d = conch.getCountGroup();
    const countNode = conch.getCountNode();
    if (this.preVetex != undefined) {
      d = d - 2;
      const last = `${this.preVetex}${this.preFps}${this.preDrawCall}`.length;
      t -= last * 4 + 72;
      if (d < 0) d = 0;
      if (t < 0) t = 0;
    }
...
var _Buffer = (function() {
  ...
  _Buffer.prototype.wab = function(arraybuffer, length, offset) {
    ...
    if (length < 0) throw 'wab error - Out of bounds';
    var i = length % 4;
    ...
    while (pos < max) {
      ...
      if (c < 0x80) {
        ...
        value += String.fromCharCode(
          ((c & 0x3f) << 6) | (bf.getUint8(pos++) & 0x7f)
        );
      } else if (c < 0xf0) {
        c2 = bf.getUint8(pos++);
        value += String.fromCharCode(
          ((c & 0x1f) << 12) | ((c2 & 0x7f) << 6) | (bf.getUint8(pos++) & 0x7f)
        );
      } else {
        c2 = bf.getUint8(pos++);
        c3 = bf.getUint8(pos++);
...
function _process() {
  ...
  this.version = '1.3.1';
  this._require = function(f) {
    console.log('process require(' + f + ')');
    return function nop() {};
  };
  this._debugObject = {};
}
...
var ErrorEvent = (function() {
  function ErrorEvent() {}
  ErrorEvent.prototype.initErrorEvent = function(
    typeArg,
    canBubbleArg,
    cancelableArg,
    messageArg,
    ...
    this.removeEventListener = function(type, listener, useCapture) {
      ...
      for (var i = 0, sz = listeners.length; i < sz; i++) {
        if (
          ...
          listeners[i].useCapture != useCapture
        ) {
      ...
      listeners.forEach(function(listener) {
        switch (evt.eventPhase) {
          ...
          case Event.AT_TARGET:
            if (listener.listener) listener.listener(evt);
            break;
...
var MouseWheelEvent = (function(_super) {
  ...
  MouseWheelEvent.prototype.initMouseWheelEvent = function(
    typeArg,
    canBubbleArg,
    cancelableArg,
    viewArg,
...
var ProgressEvent = (function(_super) {
  ...
  ProgressEvent.prototype.initProgressEvent = function(
    typeArg,
    canBubbleArg,
    cancelableArg,
    lengthComputableArg,
...
var Storage = (function() {
  ...
  Storage.prototype.create = function(url) {
    ...
    if (location.protocol == 'file:') {
      ...
      this.fileNamePre =
        this.storagePath + '/' + url.split('/')[2].replace(':', '_');
    }
...
_Cookie.cookies = [];
_Cookie.sreg = new RegExp(
  '([^=]*)\\s*=\\s*([^;]*)\\s*;\\s*(expires\\s*=\\s*(.{23,26}GMT)|)'
);
...
class KeyboardEvent extends UIEvent {
  constructor(typeArg, eventInitDict) {
    super(typeArg);
    if (eventInitDict) {
      const ini = eventInitDict;
      _this.altKey = eventInitDict.altKey;
      _this.initKeyboardEvent(
        typeArg,
        ini.bubbles,
        ini.cancelable,
        ini.view,
...
(function(layaDoc) {
  ...
  var keyEvt = function(name, code, keychar, flag) {
    this.type = name;
    this.keyCode = code;
    this.keyChar = keychar;
    this.altKey = flag & 0x1;
    this.shiftKey = (flag & 0x2) != 0;
    this.ctrlKey = (flag & 0x4) != 0;
  ...
  conch.setDeviceMotionEvtFunction(function(type, ra, rb, rg) {
    if (typeof ra != 'undefined') {
      ...
      evt.accelerationIncludingGravity = {
        x: d.getFloat32(12, true),
        y: d.getFloat32(16, true),
        z: d.getFloat32(20, true)
      };
      evt.rotationRate = {
        alpha: d.getFloat32(24, true),
        beta: d.getFloat32(28, true),
        gamma: d.getFloat32(32, true)
      };
      evt.interval = d.getFloat32(36, true);
      window.dispatchEvent(evt);
    }
  });
  setJoystickEvtFunction(function(
    type,
    thumbL_xoff,
    thumbL_yoff,
    thumbR_xoff,
...
var Navigator = (function() {
  ...
  Object.defineProperty(Navigator.prototype, 'userAgent', {
    get: function() {
      ...
      if (os == 'ios') return 'LayaBox(iPhone; CPU iPhone OS Mac OS X)';
      else if (os == 'android') return 'LayaBox Android';
      else return 'LayaBox/2.1';
    },
    enumerable: true,
    configurable: true
  });
  ...
  Navigator.prototype.addEventListener = function(
    type,
    listener,
    useCapture
  ) {};
...
var Node = (function(_super) {
  ...
  Node.prototype.appendChild = function(newChild) {
    ...
    if (
      doc &&
      newChild._z >= doc._topElement._z &&
      newChild instanceof HTMLCanvasElement &&
      !(window.document._topElement instanceof HTMLCanvasElement)
    ) {
      var ele = newChild;
      if (ele.__visible) {
  ...
  Node.prototype.cloneNode = function(deep) {
    ...
    function clone(obj) {
      ...
      for (var i in obj) {
        if (!deep) newObj[i] = obj[i];
        else newObj[i] = clone(obj[i]);
      }
      return newObj;
    }
    return clone(this);
...
(function(ns_Timer) {
  var timerobj = (function() {
    function timerobj(curtm, interval, obj, func, args, num) {
      ...
      if (Math.abs(conch.maxInterval - interval) < 1) this.interval = -1;
    ...
    this.update = function() {
      ...
      for (var sz = this.tmq.length; i < sz; i++) {
        ...
        if (
          dt < 0 ||
          ctm.interval < 0 ||
          Math.abs(conch.maxInterval - ctm.interval) < 1
        ) {
  ...
  function requestAnimationFrame(callback) {
    ...
    if (!gTimerQueue.AniFrameStTm) gTimerQueue.AniFrameStTm = Date.now();
    return id;
  }
  ...
  conch.setOnFrame(function() {
    ...
    framCount++;
    gTimerQueue.update();
    if (document) document.onframeend();
  });
...
class Location {
  constructor() {
    ...
    this.setHref = url => {
      ...
      if ((result = s.exec(url)) != null) {
        ...
        const i = _this._pathname ? _this._pathname.lastIndexOf('/') : -1;
        if (i != -1) {
          const temp = _this._pathname.substring(0, i);
  ...
  set href(url) {
    ...
    if (url != oldhref) {
      this.setHref(url);
      if (this._href != oldhref) reloadJS(true);
    }
  }
  ...
  parseQuery(url) {
    ...
    if (p1 < 0) return null;
    const q = url.substr(p1 + 1);
    q &&
      q.split('&').forEach((v, i, a) => {
        const kv = v.split('=');
        if (kv.length === 2) {
  ...
  resolve(fileName) {
    let urlcache = Location.__urlCache__[fileName];
    ...
    const basePath = this._fullpath;
    const urlfull = `${basePath}/${fileName}`;
    urlcache = Location.__urlCache__[urlfull];
    if (urlcache != null) return urlcache;
    if (!fileName.includes('://')) fileName = `${basePath}/${fileName}`;
...
var _Window = (function(_super) {
  ...
  _Window.prototype.removeEventListenernew = function(
    type,
    listener,
    useCapture
  ) {
  ...
  ColorUtil.getColNum = function(c, isRgb) {
    ...
    if (t) {
      ...
      if (i == 3 && !t[1]) {
        ...
        return (
          parseInt(Math.floor(0xff * result[3]).toString(16) + '000000', 16) +
          (ta[0] << 16) +
          (ta[1] << 8) +
          ta[2]
        );
...
var CanvasRenderingContext2D = (function() {
  ...
  Object.defineProperty(CanvasRenderingContext2D.prototype, 'globalAlpha', {
    get: function() {
      return this._saveData.curData.alpha;
    },
    set: function(v) {
  ...
  Object.defineProperty(CanvasRenderingContext2D.prototype, 'fillStyle', {
    get: function() {
      var s = this._saveData.curData,
        t = s.fillType;
      if (t == 0) return ColorUtil.toHexColor(s.fillStyle);
      else {
    ...
    set: function(fs) {
      ...
      if (typeof fs == 'string') {
        ...
        s.w = w;
        s.h = h;
      } else console.error('fillStyle 不支持此方式:' + fs);
    },
    enumerable: true,
    configurable: true
  });
  ...
  CanvasRenderingContext2D.prototype._drawText = function(
    fid,
    text,
    x,
    y,
  ...
  Object.defineProperty(CanvasRenderingContext2D.prototype, 'lineCap', {
    get: function() {
      return (
        CanvasRenderingContext2D.LINECAP[this._saveData.curData.lineCap] ||
        'butt'
      );
    },
    set: function(v) {
  ...
  Object.defineProperty(CanvasRenderingContext2D.prototype, 'lineJoin', {
    get: function() {
      return (
        CanvasRenderingContext2D.LINEJOIN[this._saveData.curData.lineJoin] ||
        'miter'
      );
    },
    set: function(v) {
  ...
  Object.defineProperty(
    ...
    'globalCompositeOperation',
    {
      get: function() {
        return (
          CanvasRenderingContext2D.BLEND[this._saveData.curData.blendType] ||
          'source-over'
        );
      },
      set: function(v) {
    ...
    get: function() {
      return (
        CanvasRenderingContext2D.TEXTBASELINE[
          this._saveData.curData.textBaseLineType
        ] || 'alphabetic'
      );
    },
    set: function(v) {
    ...
    get: function() {
      return (
        CanvasRenderingContext2D.TEXTALIGN[
          this._saveData.curData.textAlignType
        ] || 'start'
      );
    },
    set: function(v) {
  ...
  CanvasRenderingContext2D.prototype.fillBorderText = function(
    text,
    x,
    y,
    font,
    ...
    if (isNaN(fc)) {
      console.error(
        'fillBorderText fillcolor或者borderColor不符合规范' +
          fillColor +
          ',' +
          borderColor
      );
  ...
  CanvasRenderingContext2D.prototype.setSkinMesh = function(
    x,
    y,
    ib,
    vb,
  ...
  CanvasRenderingContext2D.prototype._setIBVB = function(
    x,
    y,
    ib,
    vb,
  ...
  CanvasRenderingContext2D.prototype.toBase64 = function(
    type,
    encoderOptions,
    callback
  ) {
...
CanvasRenderingContext2D.BLEND = [
  'source-over',
  'source-atop',
  'source-in',
  'source-out',
...
CanvasRenderingContext2D.PATTERN = [
  'repeat',
  'repeat-x',
  'repeat-y',
  'no-repeat'
...
var ProgramLocationTable = (function() {
  ...
  ProgramLocationTable.prototype.getFadeLocation = function(
    fadeProgramID,
    name
  ) {
...
var WebGLRenderingContext = (function() {
  ...
  WebGLRenderingContext.prototype.texImage2DEx = function(
    premultiplyAlpha,
    target,
    level,
    internalformat,
  ...
  WebGLRenderingContext.prototype.texSubImage2DEx = function(
    premultiplyAlpha,
    target,
    level,
    xoffset,
    ...
    conch.bf.wu32(arguments[0] ? 1 : 0);
    var argobj =
      arguments[6 + 1] && (arguments[6 + 1]._nativeObj || arguments[6 + 1]);
    if (argobj) {
      if (argobj && argobj.isCanvas && argobj.isCanvas()) {
        var e = new Error('dd');
  ...
  WebGLRenderingContext.prototype._uniformMatrixfv_x = function(
    cmd,
    location,
    transpose,
    values,
  ...
  WebGLRenderingContext.prototype.bindFramebuffer = function(
    target,
    framebuffer
  ) {
  ...
  WebGLRenderingContext.prototype.getProgramParameterEx = function(
    vs,
    ps,
    define,
    pname
  ...
  WebGLRenderingContext.prototype.getActiveAttribEx = function(
    vs,
    ps,
    define,
    index
  ...
  WebGLRenderingContext.prototype.getActiveUniformEx = function(
    vs,
    ps,
    define,
    index
...
var HTMLCanvasElement = (function(_super) {
  ...
  HTMLCanvasElement.prototype.onCSS_Transform = function(mat) {
    ...
    if (sw == 0 || sh == 0) {
      if (mat[4] > mat[5]) {
        console.warn(
          '设置横竖屏错误! 请在初始html页面meta中设置screenorientation为横屏landscape'
        );
      } else {
        console.warn(
          '设置横竖屏错误! 请在初始html页面meta中设置screenorientation为竖屏portrait'
        );
      }
    }
  };
  ...
  HTMLCanvasElement.prototype.toBase64 = function(
    type,
    encoderOptions,
    callback
  ) {
...
var HTMLInputElement = (function(_super) {
  ...
  function HTMLInputElement() {
    ...
    _this.setCursorPosition = _this._nativeObj.setCursorPosition.bind(
      _this._nativeObj
    );
  ...
  HTMLInputElement.prototype._addEventListener = function(
    type,
    listener,
    useCapture
  ) {
...
var HTMLAudioElement = (function(_super) {
  ...
  HTMLAudioElement.prototype._addEventListener = function(
    type,
    listener,
    useCapture
  ) {
    ...
    this._nativeObj.addEventListener(type, function(e) {
      var ev = new _lbEvent(type);
      ev.target = this;
      if (type == 'canplaythrough') _t.readyState = 4;
      _t.dispatchEvent(ev);
    });
  };
...
var Document = (function(_super) {
  ...
  Document.prototype._downloadOk = function(d) {
    ...
    for (
      var i = this._evalNum, len = this.scriptTextList.length;
      i < len;
      i++
    ) {
  ...
  Document.prototype._dispatchEvent = function(evt) {
    var ancestores = null;
    if (evt.target) ancestores = evt.target.getAncestorsNode();
    ...
    var stop = (function() {
      window.dispatchEvent(evt);
      if (evt._propagationStopped) return true;
  ...
  Document.prototype.getElementsByTagName = function(tagname) {
    var d = new NodeList();
    if ('body' == tagname) d.push(this.body);
    else if ('head' == tagname) d.push(this.head);
    return d;
  };
...
var CloseEvent = (function(_super) {
  ...
  CloseEvent.prototype.initCloseEvent = function(
    typeArg,
    canBubbleArg,
    cancelableArg,
    wasCleanArg,
...
var DOMParser = (function() {
  ...
  DOMParser.prototype.getResult = function() {
    if (!this._result) {
      ...
      this._result.childNodes[0] = DOMParser.initXMl(
        this._parser.getResult().childNodes[0]
      );
    }
    return this._result;
  };
...
var _jsXmlNode = (function(_super) {
  ...
  _jsXmlNode.prototype.getElementsByTagName = function(name) {
    var result = [];
    if (this.nodeName == name) result.push(this);
    else {
...
(function(GL_CAPS) {
  ...
  GL_CAPS[(GL_CAPS['TEXTURE_COMPRESSION_ETC1'] = 4)] =
    'TEXTURE_COMPRESSION_ETC1';
  GL_CAPS[(GL_CAPS['TEXTURE_COMPRESSION_ETC2'] = 8)] =
...
conch.onerror = function(message, filename, lineno, colno, error) {
  if (window.onerror) {
    ...
    window.onerror(
      mg == 'undefined' ? undefined : mg,
      fn == 'undefined' ? undefined : fn,
      ln != 'undefined' ? parseInt(ln) : undefined,
      cn != 'undefined' ? parseInt(cn) : undefined,
      e
    );
  }
};
