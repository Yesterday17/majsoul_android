...
var regex_resversion = /resversion.+\.a\.json$/;
...
var XMLHttpRequest = (function(_super) {
  ...
  XMLHttpRequest.prototype.open = function(type, url, async) {
    console.log('[XHR-OPEN][' + type + '] url = ' + url);
    if (!url) return;
    ...
    this._url = (() => {
      var result;
      if (regex_resversion.test(url)) {
        return 'RES_VERSION';
      } else if (regex_resource.test(url)) {
        result = regex_resource.exec(url);
  ...
  XMLHttpRequest.prototype.send = function(body) {
    console.log('[XHR-SEND] url = ' + this._url);
    if (body) {
      if (
    ...
    if (this._url === 'RES_VERSION') {
      console.log(window.resversion);
      this._response = window.resversion;

      this._readyState = 4;
      this._status = 200;
      this.xhr._changeState(4);
      if (this._onloadcb) {
        this._loadsus();
      }
    } else if (this._method == 'POST' && body) {
